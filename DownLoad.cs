    ...
    public class DownLoad
    {
        ...
        public static bool DownloadFile(HttpContext httpContext, string filePath, long speed)
        {
            ...
            try
            {
                ...
                int fileLengths = (int)fileLength;
                ...
                try
                {
                    ...
                    {//------如果是续传请求，则获取续传的起始位置，即已经下载到客户端的字节数------   
                        ...
                        startBytes = Convert.ToInt32(range[1]);//已经下载的字节数，即本次下载的开始位置     
                        if (startBytes < 0 || startBytes >= fileLength - 1)
                        {//无效的起始位置   
                            return false;
                        }
                    }
                    ...
                    //{//客户端中断连接，则暂停   
                    //System.Diagnostics.Debug.WriteLine(httpContext.Request.Headers["Range"] + " ------------" + i);
                    httpContext.Response.BinaryWrite(br.ReadBytes(fileLengths - startBytes));
